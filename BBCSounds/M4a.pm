...
sub getStartOffset {
	...
	my $streamStartTime = ($props->{startNumber} - 1) * ( $props->{segmentDuration} / $props->{segmentTimescale} );
...
sub getAudio {
	...
	while ( $v->{need} <= length $v->{inBuf} ) {
		...
		if ( $v->{mdat} ) {
			...
			main::DEBUGLOG
			  && $log->is_debug
			  && $log->debug( "extracted ", length $v->{mdat}->{data}," bytes, out ", length $v->{outBuf} );
			$v->{mdat} = undef;
		}
	}
...
sub process_mp4a_atom {
	...
	$result{'reserved2'} = [decode_u32( substr( $data, 8,  4 ) ),decode_u32( substr( $data, 12, 4 ) )];
...
sub mp4esdsToADTSHeader {
	...
	$profile = 2
	  if ( $profile == 5 );    # Fix because Touch and Radio cannot handle ADTS header of AAC Main.
